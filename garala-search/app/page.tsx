...
export default function Home() {
  ...
  async function fetchAds() {
    ...
    // NOUVEAU : On sélectionne toutes les colonnes de 'ads' ET le nom du groupe lié
    let query = supabase
      .from('ads')
      .select('*, groups(name)')
    ...
    if (search) {
      ...
      // Note : rechercher dans le groupe nécessite une syntaxe spécifique, restons simple pour l'instant
    ...
    if (error) {
      ...
      // TypeScript a parfois du mal avec les jointures profondes, on force le type
      setAds((data as any[]) || []);
    }
    setLoading(false);
  }
            ...
            {ads.map((ad) => (
              <div key={ad.id} className="bg-white rounded-2xl overflow-hidden shadow-sm hover:shadow-xl transition-shadow duration-300 border border-gray-100 flex flex-col">
                ...
                <div className="p-5 flex-1 flex flex-col">
                  ...
                  {/* NOUVEAU : Nom du groupe source */}
                  {ad.groups?.name && (
                    <div className="mb-2 inline-flex items-center gap-1 px-2 py-1 bg-gray-100 rounded text-[10px] font-bold text-gray-500 uppercase tracking-wider">
